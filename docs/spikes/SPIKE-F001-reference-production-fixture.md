...
1. Assemble bodies, eyes, mouths, hands, and expressions from **Kenney Shape Characters** (CC0). The archive holds 104 separated parts, each at two resolutions; the build uses the `Double` set.
...
1. Derive a risk catalogue from the charter, v1 requirements, quality system, competitive research, and repository reviews. **Done — `docs/quality/fixture-risk-matrix.md`, 70 risks.**
2. Map each material risk to at least one isolated fixture assertion. **Done in the same document. Six residual risks are unassertable after their assertable parts were split out, two entries are fixture-construction rules audited by reading rather than by execution and are never counted as coverage, and every requirement is either covered or excluded with a reason.**
3. Design an original production brief and cast sufficient to exercise the required archetypes. **Done.** Cast: `fixtures/`, 141 part files holding 84 unique images, covering biped ×3, quadruped, vehicle, prop and environment, with joints, a rig tree, and deformation meshes. Brief: `fixtures/brief/`, two original productions at 168 s and 156 s that differ on storyline, focus and concept, with the Gate 1 fields and the stimulation profile carried as data in `creative-intent.json`. `fixtures/tools/check_brief.py` refuses any rig reference the cast cannot perform, any shot list that does not sum to its declared duration or leaves a gap, and any pair of productions that fails to differ on all three narrative axes. **Both productions are blocked on original audio that does not yet exist — see the brief's Audio dependency.**
4. Record provenance and license compatibility before asset work begins, and decide per asset whether it is originated, commissioned, or adapted from a compatibly licensed source. `RGT-O001` fixed the constraint — official assets must be redistributable — and left the route per asset to this step. **Done for the cast — `fixtures/PROVENANCE.json`, tracked, both sources CC0 and adapted. The generator reads its inputs out of the verified archive rather than from any extracted copy, and checks each member against its own recorded SHA-256, so the bytes the build consumes are the bytes the licence record describes.**
