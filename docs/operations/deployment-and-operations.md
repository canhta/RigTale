...
Exact platform versions, hardware floors, performance thresholds, packaging model, renderer process, server language, storage engine, and cloud providers remain evidence-pending. `SPIKE-CS001` owns local contract storage, migration, corruption, backup, and restore evidence; `SPIKE-R001` owns renderer workload and headless evidence; `SPIKE-I001` owns macOS process and packaging evidence; and `SPIKE-M001` owns MCP and embedded-provider operational evidence without blocking the core local renderer path.
