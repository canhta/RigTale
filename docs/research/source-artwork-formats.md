...
|---|---|---|---|---|---|---|---|---|---|---|---|
| **OpenRaster `.ora`** | Yes, v0.0.6, with a RelaxNG schema | ZIP + XML + PNG | **met** | yes (nested `stack`) | 20 `composite-op` values, defined by reference to W3C Compositing-1 | **no** | **no** (a proposal covers per-PNG colour-management chunks only) | **no** (prose refers to a `text` element that does not exist in the schema) | not specified normatively | **no** (proposal only) | Krita, GIMP, MyPaint only |
...
- **pyora** — `https://gitlab.com/inklabapp/pyora`, MIT (`LICENSE`, "Copyright (c) 2019 Paul Jewell"), PyPI 0.3.11 uploaded 2021-03-19, clone `54e16cbea6d0a5ef4a50b429f98c1e03489493d9` (2021-03-18). Writer at `pyora/Project.py:358`. **Unmaintained for five years, and non-conformant in two visible ways**: it declares `ORA_VERSION = "0.0.1"` (`pyora/__init__.py:3`) and emits `src` values with a leading slash — `f'/data/layer{filename_counter}.png'` (`Project.py:392`) — where the spec specifies root-relative paths of the form `data/layer2.png`.
...
**Every writer degrades the file, verified in source.** GIMP's `plug-ins/python/file-openraster.py` (master `b73d576491694a2a3404d9d7a3c511228222430e`, 2026-08-03) maps roughly sixteen GIMP modes to real `svg:*` values and **collapses thirty further modes — Dissolve, Behind, Divide, Subtract, Grain extract/merge, Color erase, Erase, Merge, Split, the four LCH modes, Vivid/Pin/Linear light, Hard mix, Exclusion, Linear burn, Luma darken/lighten only, Luminance and Pass-through — to `svg:src-over`**, with an in-source `# FIXME Determine the closest available layer mode`. Krita's `plugins/impex/ora/kis_open_raster_stack_save_visitor.cpp` writes unmapped ops as `"krita:" + compositeOpId` (non-portable), smuggles alpha-lock as `svg:src-atop` or a non-baseline `alpha-preserve` attribute, writes adjustment layers as a `<filter type="applications:krita:NAME">` element **with no data payload**, rasterises clone/generator/vector layers through `layer->projection()`, never visits masks at all (they are baked into the projection), and declares `<image version="0.0.1">`.
...
**Who exports it.** `openraster.org` states "Applications that support OpenRaster are Krita, GIMP, MyPaint and Scribus." MyPaint's `gui/filehandling.py:592-593` shows `ext2saveformat` limited to `.ora`, `.png`, `.jpeg`, `.jpg` — **ORA is MyPaint's only layered format; it has no native format of its own.** Krita's own manual calls ORA "an interchange format… designed to replace \*.psd as an interchange format" (`https://docs.krita.org/en/general_concepts/file_formats/file_ora.html`). **No rigging or animation tool screened in §1.12 accepts ORA.**
...
**Specification: published, and the repository's prior characterisation of it is wrong.** `https://www.adobe.com/devnet-apps/photoshop/fileformatashtml/` returns HTTP 200 and is titled "Adobe Photoshop File Formats Specification — November 2019". Every quotation and count taken from it here is verified against the specification document itself, 50 pages, SHA-256 `14fa01d426fe46d627dedc0616e8bd8de21d2eb81c96ad4c0b29171510eed43a`, held locally and not redistributable. It states its purpose verbatim: "This document is provided for 3rd parties to read and write the Photoshop native file format." It covers **both PSD and PSB** in one document.
...
**`PR-A003` write gate: MET.** Two actively released libraries were verified to both read and write; only one of them can author a document from scratch:
...
- **ag-psd** — `https://github.com/Agamnentzar/ag-psd`, MIT (`LICENSE` read: "The MIT License (MIT) … Copyright (c) 2016 Agamnentzar"), npm `31.0.2` published 2026-07-02, clone `387049670cb89b88fb8fe1b7c01aeacf98dd2e3b` (2026-07-02). Write API at `src/index.ts:29,35,41` — `writePsd`, `writePsdUint8Array`, `writePsdBuffer`. **PSB is written via `WriteOptions.psb` (`src/psd.ts:1859-1860`, "Saves document as PSB (Large Document Format) file"), with the version word set at `src/psdWriter.ts:260`.** Note that `README.md:15` still claims "Does not support The Large Document Format (8BPB/PSB)" — that line is stale and is contradicted by the project's own `CHANGELOG.md` v12.1.0 entry and by the code; the library is at v31.
  ...
  **Headless operation is confirmed by reading the source, not the README.** `src/helpers.ts:385-386` makes `createCanvas` throw unless `initializeCanvas` is called, and `README.md:73` says node-canvas is needed "to read image data and thumbnails". But layer pixels can be supplied as raw `imageData` (`Layer.imageData?: PixelData`, `src/psd.ts:1774`), the writer consumes it directly (`src/psdWriter.ts:211,245`), and the only *unconditional* canvas call on the write path is `createCanvas` inside thumbnail generation (`src/psdWriter.ts:585`); the remaining `getContext('2d')` calls (`:248`, `:619`, `:729`) are reached only when a `canvas` is supplied instead of `imageData`. **With `imageData` and no `generateThumbnail`, ag-psd writes PSD and PSB in plain Node with no canvas, no display, and no Adobe software.**

  Structure it can write, from `src/psd.ts` and the handler tables in `src/additionalInfo.ts`: layer tree and groups (`children`, `'lsct'`), opacity (`opacity`, `'iOpa'`), the 31-value `BlendMode` union (`src/psd.ts:1-5`) — the 28 PSD keys plus `'linear height'`, `'height'` and `'subtraction'`, three names with no four-character key in `fromBlendMode` that therefore cannot survive a write — raster masks (`mask`, `realMask`, `'LMsk'`), clipping (`clipping`), adjustment layers (`adjustment`, with handlers for `'brit' 'levl' 'curv' 'expA' 'vibA' 'hue2' 'blnc' 'blwh' 'phfl' 'mixr' 'clrL' 'nvrt' 'post' 'thrs' 'grdm' 'selc' 'CgEd'`), vector fill/stroke/mask, layer effects, and **no ICC profile at all**. The resource-1039 handler at `src/imageResources.ts:1228-1239` is guarded by `MOCK_HANDLERS &&`, and `src/helpers.ts:5` sets `export const MOCK_HANDLERS = false;`, so it is never registered — one of thirteen handlers disabled the same way. Unregistered image resources are discarded on read (`src/psdReader.ts:284`, `skipBytes(reader, left())`) and there is no writer for them. The only ICC-adjacent field ag-psd handles is the `iccUntaggedProfile` boolean (resource 1041, `:1219-1224`). The specification defines 1039 as "The raw bytes of an ICC (International Color Consortium) format profile", so **a PSD read and rewritten through ag-psd loses its colour profile.** Routed to `SPIKE-A002` and to the colour-and-alpha defect below.
...
- **ImageMagick** documents `PSD | RW` and `PSB | RW` at `https://imagemagick.org/formats/`, with `coders/psd.c` containing `WritePSDImage` (:186) and a blend-mode round-trip table (`PSDBlendModeToCompositeOperator`, :858-909). Authoring an arbitrary layer tree from scratch through ImageMagick is **not verified**; the documented write path is oriented toward round-tripping an input PSD.
...
`SPIKE-C001` originally stated that "`.kra` and `.ora` are open ZIP+XML containers a program can write", and now carries the correction at `:51`. **For `.ora` the original was correct. For `.kra` it was not, on both halves of `PR-A003`.**

**There is no published specification.** The only user-facing document is `https://docs.krita.org/en/general_concepts/file_formats/file_kra.html`, whose entire technical content is that `.kra` "is Krita's internal file-format", its "construction is vaguely based on the open document standard, which means that you can rename your `.kra` file to a `.zip`", it contains a `mergedimage.png`, and — verbatim — "**Other applications mostly cannot open `.kra` files.**" The Krita repository (master `57180ec75d8df25073d4dbb99fa5d44b08d3e266`, 2026-08-02) does carry one machine-readable artefact: `krita/dtd/krita.dtd`, 93 lines, a DTD for `maindoc.xml` declaring `DOC`, `IMAGE`, `LAYERS`, `layer` and `FILTERCONFIG`. It is not a substitute for a specification — it still calls the format "the KImageShop image format", fixes `syntaxVersion` as "always 1 for now", marks `width`, `height` and `linked` OBSOLETE, and covers none of masks, `compositeop`, `passthrough`, keyframes, the ZIP layout, or the LZF tile encoding, all of which the current writer emits. **Everything the DTD omits is defined only by the implementation.** It therefore fails the word "documented" in `PR-A003` before the write gate is even reached.
...
**`PR-A003` write gate: MET, with caveats.** GIMP documents true headless batch operation — `-i, --no-interface` and `-b, --batch=commands`, plus a `gimp-console` binary (`https://docs.gimp.org/3.0/en/gimp-fire-up.html`) — so `gimp-console -i -b '(...)'` writes XCF with no GUI. But that is GPL-3.0-or-later software driving a Scheme PDB, which is an operational dependency, not a library. The one permissive third-party writer is **xcf-rs** (`https://github.com/mothsART/xcf-rs`, `UNLICENSE` file present and `license = "Unlicense"`, crates.io crate `xcf-rs` created 2025-06-08 with 0.5.0 its only version, clone `3edb3c98bd20c048b8a278325956622b0bb6d7a5`). It is a fork of `shivshank/xcf-rs`, whose crate is named `xcf` and stopped at 0.4.0, also Unlicense — the two are separate crates, not two versions of one, whose `src/create.rs:670 pub fn save(...)` is real but whose own README states the reader "supports RGB or RGBA images, but not grayscale or indexed" and that better-compressed XCF is "not supported". **gimpformats** (LGPL-3.0, clone `d3181b81a1accab51f6e892ec019ac0664ec83fc`, 2026-06-21) lists "Saving" under its own README heading "**In progress but results in crashes and tests failing**". **xcftools** is read-only and mixed-licence: Makholm's own code is public domain, but the three bundled GIMP headers carry three different licences — `gimp/xcf-private.h` GPL-3.0-or-later, `gimp/base-enums.h` GPL-2.0-or-later, `gimp/gimpbaseenums.h` LGPL-2.0-or-later — so the distributed whole is GPL-3.0-or-later encumbered.
...
**It does not need to be met.** Procreate documents "Export your artwork as a native .procreate file or a layered Adobe® Photoshop® PSD. This preserves your layers, layer names, opacity, visibility, and blend modes" (`https://help.procreate.com/procreate/handbook/actions/actions-share`), and separately "You can also share layers as PNG Files, and Animated PNGs" (`https://help.procreate.com/procreate/handbook/gallery/gallery-file-types`). **The documented layered egress is PSD.**
...
**Egress is PSD/PSB**, quoted in §1.4. One fidelity caveat worth carrying into `SPIKE-A002`, from the `clip_to_psd` README: **the converter can export vector, tone and frame-border layers only as the rasterised pixels the `.clip` happens to hold**, and its author reports that "It seems newer versions of CSP drop the pixel data for Vector layers" — a vendor behaviour that is `not verified` against any Celsys source, and which would make those layers unrecoverable by third-party converters. That is a real ingestion failure mode with a named cause.
...
**SVG is the vector counterpart and has no layer concept.** SVG 2 is a W3C Candidate Recommendation; its date is recorded as 4 October 2018 but is **not verified** — see Open / Not Verified. **The word "layer" does not occur anywhere in its structure chapter** (`https://www.w3.org/TR/SVG2/struct.html`), counted over the 40-page chapter document, SHA-256 `02207470c86ccafea4b6a675aa0079c03fb7f285b3fe00c2b3245db2ea230b3a`; grouping is `<g>`, and "layers" in Inkscape are `<g>` elements carrying vendor attributes. Blending comes from CSS `mix-blend-mode` and `isolation`, i.e. the same W3C Compositing-1 sixteen. Writing SVG headlessly is trivial. **The decisive limitation is in §2.5: SVG 2 contains no mesh or texture-mapping primitive at all** — zero occurrences of "mesh" in the specification index or the paint-servers chapter.
...
- **Blend modes are lossy on the way in.** `source/creator/io/psd.d:55-70` maps fourteen PSD blend keys and sends **everything else to `BlendMode.Normal` via `default:`**. Of the 28 keys Adobe documents, fourteen are unmapped; thirteen of those are real blend modes silently normalised, and the fourteenth, `pass`, is a group pass-through the group branch reads separately at `psd.d:195`.
...
- **Layers are premultiplied at import.** `source/creator/io/kra.d:209` calls `inTexPremultiply(...)` on the extracted layer data before creating the texture, and `psd.d:204` does the same on the PSD path. The source data is straight alpha; the runtime is premultiplied; the conversion is 8-bit.
- **Opacity, visibility, and position survive** (position `kra.d:221-225`, visibility and opacity `kra.d:228-229`), and draw order becomes `zSort = -(index)` (`kra.d:237`).
...
| OpenToonz | "Photoshop documents (PSD files) can be loaded as a scene element… taking into account the layers"; "Supported formats are RGB or grayscale images, using 8 or 16 bits per channel" — both verbatim from `opentoonz_docs` `source/working_in_xsheet.rst:246,248` at `d0cb6a49f3b489c455bc6cf7674ff98f5682dccf` |
...
| Blender Grease Pencil | **Neither — SVG only.** Grease Pencil IO is SVG import plus SVG/PDF export (`source/blender/io/grease_pencil/intern/`); `R_IMF_IMTYPE_PSD` survives in the image-type enum but is wired to nothing beyond a filename extension (`blenkernel/intern/image_format.cc:555`), so no PSD can be read or written |
...
**PSD.** Three GitHub advisories against `psd-tools`, quantified in §1.4, one of which is an **arbitrary file write** reachable from a crafted smart-object filename and was published 24 days before this screening. ImageMagick's PSD coder has CVEs on both the read and write paths.

**Comparative exposure of the compositing candidates**, from OSV queries against the Debian 12 source packages on 2026-08-02 — all-time counts across all severities, offered as an order-of-magnitude signal and not as a defect-density metric: `imagemagick` 739 advisory records, `tiff` 271, `vips` 22. Pillow's advisory list is long and current, including four 2026 advisories for missing `_decompression_bomb_check()` calls (`GHSA-45hq-cxwh-f6vc`, `GHSA-5x94-69rx-g8h2`, `GHSA-8v84-f9pq-wr9x`, `GHSA-phj9-mv4w-65pm`), two further 2026 decompression-bomb denial-of-service advisories (`GHSA-whj4-6x5x-4v2j`, `GHSA-jjj6-mw9f-p565`), and an OS-command-injection advisory in `WindowsViewer.get_command()` (`GHSA-4x4j-2g7c-83w6`).
...
- `[FACT]` **Clip Studio Paint** is hybrid: it has vector layers, but its own manual says the way to move layers to another application is PSD/PSB, and `clip_to_psd`'s README records that vector, tone and frame-border layers survive conversion only as rasterised pixels held inside the `.clip`, which newer CSP versions are reported to drop (§1.8).
...
- `[REPORTED]` Vendor-published usage figures, primary for the vendor's own claim and **not** market share: MediBang Paint 100 million downloads (2024). Figures circulated for Clip Studio Paint ("exceeded 50 million copies", 2025-04-10) and Procreate ("over 30 million users", 2023) could not be traced to a vendor publication and are **not verified**; they are not used here.
...
|---|---|---|
| Inochi2D / Inochi Creator | `ExPart` built from a `Texture` created from decoded PSD/KRA layer pixels | `source/creator/io/kra.d:204-211` and `source/creator/io/psd.d:204-206` at `dba60811` |
...
| GEGL (`operations/generated/`) | yes | yes — but normal is `gegl:over` in `operations/common/over.c`, not in the generated set | **none** | `gegl:subtract` and `gegl:divide`, generated by `math.rb` as arithmetic point-composers rather than by `svg-12-blend.rb`; no other PSD-only mode |
| OpenImageIO (`imagebufalgo.h:1005-1044`) | `over`/`zover` only | **none** | **none** | no |
| Pillow (`src/PIL/ImageChops.py`) | partial | **8 of 12** — no normal, color-dodge, color-burn, exclusion | **none** | `add` and `subtract` only, both with `scale`/`offset` parameters; no divide |
...
**This is a `PR-R003` and `PR-R005` problem, not a nice-to-have.** If a user's PSD uses Linear Light on a shadow layer, a Skia, tiny-skia, Cairo, pixman, libvips, or raqote backend has nothing to map it to. The two options are to implement the missing modes against a reverse-engineered reference (Krita's and ImageMagick's implementations are the available oracles — Krita is GPL-3.0 copyleft, read from its `COPYING`, while ImageMagick ships its own bespoke "ImageMagick License", so only the ImageMagick oracle can be read without copyleft consequences), or to declare a **supported blend-mode profile** and fail loudly on anything outside it — which is exactly the `PR-C003` "never silently substitute a visibly incorrect action" posture applied to compositing. Inochi Creator's `default: blendMode = BlendMode.Normal` is the failure mode to avoid, and it is in shipped software.
...
**ImageMagick** — `https://github.com/ImageMagick/ImageMagick`. **The ImageMagick License**, a bespoke Apache-2.0-derived document, not Apache-2.0 itself. Clone `cc6d9251e7746dcb9ef6b391976c2a334edd0d15`, 2026-08-02; release 7.1.2-29, 2026-07-27, on a roughly weekly cadence. **The only candidate that both composites with the extended Photoshop blend set and reads and writes layered PSD** (`coders/psd.c`, `ReadPSDImage` :2392, `WritePSDImage` :186, blend map :858-909). Alpha: straight in the pixel model, premultiplied internally during compositing (`MagickCore/composite.c:151,1113-1119`). Against it: the largest CVE surface of any candidate (739 Debian advisory records), output that depends on build-time QuantumDepth and HDRI settings, and no determinism claim. **Its strongest role is as a reference oracle and a conversion tool, not as the production compositor.**

**GraphicsMagick** — `http://www.graphicsmagick.org/` (the site serves HTTP only; HTTPS does not connect). MIT-style licence, read from `Copyright.html`. Release 1.3.48, 2026-07-23, from the project's own `NEWS.html`. **No clone was taken** — the project is Mercurial and the git mirror 404s — so no source was read, and blend-formula conformance and alpha semantics remain **not verified**. **PSD was not dropped.** 1.3.24 (2016-05-30) classified PSD as "broken" (until it is fixed), excluded it from the default build, and left it reachable behind the `--enable-broken-coders` configure option. Weakest evidence base of the set.

**OpenImageIO** — `https://github.com/AcademySoftwareFoundation/OpenImageIO`. Apache-2.0 (`LICENSE.md`). Clone `3aab3be78659eceb782b9b823273dc16eac5eae0`, 2026-07-31; release v3.1.16.0, 2026-07-30. **Decisive negative: it has no PDF or Photoshop blend modes at all.** `src/include/OpenImageIO/imagebufalgo.h:1005-1044` offers only `over()` and `zover()`, and a case-insensitive grep across `src/` for `softlight|colorburn|hardlight|blendmode|overlay` returns three hits, none of them a blend mode: `overlay_color_space`, a PSD global-mask field, at `src/psd.imageio/psdinput.cpp:181,1885`, and a data-window overlay in the viewer UI at `src/iv/imageviewer.h:308`. Alpha is associated/premultiplied with an explicit `premult`/`unpremult`/`repremult` trio. **Excellent as an image I/O and colour-pipeline layer; not a layered-artwork compositor.**

**Blend2D** — `https://github.com/blend2d/blend2d`. zlib (`LICENSE.md`). Clone `6dbc2cefbc996379e07104e34519a440b49b15d7`, **2025-11-29 — roughly eight months without a commit**. **`git ls-remote --tags` returns nothing: the repository has never cut a tagged release**; the in-source version is `BL_VERSION 0.21.2`. All twelve W3C separable modes — normal is `BL_COMP_OP_SRC_OVER = 0` — plus LinearBurn/LinearLight/PinLight, but **no hue, saturation, colour, or luminosity** (`BL_COMP_OP_MAX_VALUE = 28`). Its JIT pipeline has a documented off switch (`BL_CONTEXT_CREATE_FLAG_DISABLE_JIT`), but **no claim that the JIT and reference pipelines agree bit-for-bit** — which sharpens rather than resolves the concern already recorded in `landscape.md`.

**Pillow** — `https://github.com/python-pillow/Pillow`. MIT-CMU (`LICENSE`). Clone `425a038b89afc76f145b522fd5e399385b56b09b`, 2026-08-02; release 12.3.0, 2026-07-01. **Blend coverage is the weakest of any live candidate**: `ImageChops` has eight of the twelve W3C separable modes and **none** of the four non-separable ones. The four absent are normal, color-dodge, color-burn and exclusion — normal only in the sense that source-over lives on `Image.alpha_composite` rather than in `ImageChops`. Its PSD reader explicitly discards blend data — `src/PIL/PsdImagePlugin.py:248`, `# skip over blend flags and extra information`. Large and current advisory list. **Reject as a compositor; retain only as a codec convenience.**

**GEGL** — `https://gitlab.gnome.org/GNOME/gegl`. Split licence: library LGPL-3.0-or-later (`gegl/gegl.h:4-6`), repository `COPYING` GPL-3.0. Clone `2d0e992c970e784ed0fa8331785941ccf859aa32`, 2026-07-31; release `GEGL_0_4_70`, 2026-03-24. All twelve W3C separable modes — eleven generated from `operations/generated/svg-12-blend.rb` citing SVG 1.2, and normal as `svg:src-over` in `operations/common/over.c:208`; **no hue/saturation/colour/luminosity in GEGL itself** — those live in GIMP's own `app/operations/layer-modes`, outside this repository and **not verified here**. Premultiplied via babl `RaGaBaA`. Copyleft; `reference` only.
...
**Aseprite `src/doc`** — the blend implementation specifically is **MIT** (`src/doc/LICENSE.txt`; `src/doc/blend_mode.h:4-5` "This file is released under the terms of the MIT license"), inside a repository whose root `EULA.txt` is proprietary and forbids redistribution. Clone `717ab76b2ed9b814fda4b65eb388f6ad480ca4ee`, 2026-07-30; release v1.3.18.1, 2026-07-23. Complete W3C set — the separable modes in 8-bit integer arithmetic, but soft light and all four non-separable modes in double precision — with the canonical PDF helpers `lum()`, `clip_color()`, `set_lum()` (`src/doc/blend_funcs.cpp:362-481`), and W3C Compositing-1 and the Adobe PDF reference cited in-source at `:12-13`. A float oracle is a different determinism proposition from an integer one. **Its value to RigTale is as a compact, permissively licensed cross-check oracle, not as a dependency** — and the licence boundary must be stated precisely if it is ever used, because the surrounding application is not open source.

**Krita `KoCompositeOp`** — the widest blend coverage of anything inspected in this screening (148 composite ids, §2.4), and the only inspected source that makes the Photoshop-versus-SVG formula divergence explicit. **Krita is a Qt desktop application; no headless library extraction path was verified.** GPL-family licence, not verified by reading the file. `reference` only, and a strong one.

**OpenColorIO** — `https://github.com/AcademySoftwareFoundation/OpenColorIO`. **BSD-3-Clause**, read from `LICENSE` at the pinned tag. Release `v2.5.2` = `c52966a6677723d5bd2dbef0ccec3fed9cbc3790`, 2026-05-13. **Not a compositing library and contributes zero blend modes** — but blend results are colour-space dependent, so it is the natural home for the working-space half of the `TODO.md` colour-management gap.
...
**No candidate in this set makes an output-determinism claim about its compositing path.** Thirteen clones were searched for "deterministic" and "reproducib"; ten returned hits, and all but three are off-topic — contributor process (libvips `CONTRIBUTING.md`), fuzzing determinism (ImageMagick `MagickCore/random.c:376,901`, `MagickCore/image.c:191`, and OpenImageIO's testsuite), test-harness PRNGs (pixman `test/utils/utils-prng.h:51`) and scripting state (Aseprite `src/app/app.cpp:652`). Three hits are material and are recorded rather than discarded: GEGL claims reproducibility for its **noise source**, not its compositing — "GEGL provides functions to generate deterministic random numbers, based on a seed and on 2D coordinates. It allows to reproduce the same sequence of random numbers in both CPU and GPU devices" (`gegl/gegl-random.h:30-32`); GEGL's own test harness classifies operations whose "reference is not reproducible" (`tools/gegl-tester.c:365`); and tiny-skia declines an optimisation in its raster pipeline because it "breaks reproducible result" (`src/pipeline/lowp.rs:851`), a design statement about output stability rather than a guarantee. Cairo, raqote and libmypaint returned no hits at all. This is stated as a search result over the sources inspected, not as a universal claim, and the thirteen clones exclude four members of §2.6 — Skia, GraphicsMagick, Krita and OpenColorIO — which were not searched.
...
| **GraphicsMagick, libmypaint** | `reject` | PSD disabled by default as a "broken" coder since 1.3.24 and no source read / brush-dab library with no PDF blend modes. | — |
| **raqote** | `defer`, watch-list only | Complete blend set and CPU-only, but eighteen months stale and below tiny-skia on every axis. Revisit only if tiny-skia is ruled out. | `SPIKE-R001` |
...
| PSD defines 28 blend keys, publishes no formulas, and no general 2D library implements the extra eleven (§2.4) | new requirement `PR-R008`, cross-referenced from `PR-R003` and `PR-R005` | Create `PR-R008` — a required **supported blend-mode profile** with explicit failure on out-of-profile modes, mirroring `PR-C003` — and point `PR-R003` and `PR-R005` at it | `applied` |
| Cairo makes no determinism statement in its repository at all; `candidate-screening.md` recorded a "marketing line about consistent output" that does not exist (§2.6) | `docs/research/candidate-screening.md` Cairo entry | Replace the marketing-line sentence with the zero-hit grep result over clone `bd04e43e` | `applied` |
| ag-psd discards the ICC profile: the resource-1039 handler is `MOCK_HANDLERS`-guarded and unregistered resources are skipped on read, so a PSD round-tripped through ag-psd loses its colour profile (§1.4) | `SPIKE-A002`, and the colour-management gap in `TODO.md` | Record ICC loss as a required ingestion fixture case and as a constraint on any ag-psd-based write path | `applied` |
...
1. **`docs/spikes/SPIKE-C001-competitive-landscape.md` originally said `.kra` and `.ora` are alike — open ZIP+XML containers a program can write.** Half wrong, and **corrected in place at `:51`**. ORA has a published specification, a RelaxNG schema, and at least two independent writers; `.kra` has **no published specification anywhere**, is defined only by Krita's implementation, stores pixels as binary LZF tiles rather than XML-referenced PNGs, and has exactly one third-party writer at version 0.0.0 with no LICENSE file that writes a mimetype string disagreeing with Krita's own constant. §1.5.

2. **The same sentence called PSD reverse-engineered.** Imprecise, and **corrected in place at `:51`**. Adobe publishes "Adobe Photoshop File Formats Specification — November 2019" covering PSD and PSB, explicitly "provided for 3rd parties to read and write the Photoshop native file format". What is genuinely unpublished is *semantics* — the specification states "This document does not explain how to interpret the data" — and, critically, **the blend-mode formulas**. The correct statement is that PSD's syntax is documented and its behaviour is not. §1.4.

3. **`docs/spikes/SPIKE-C001-competitive-landscape.md:53` — "the correct candidate set is a layered-raster compositing set that was never enumerated", implying the vector shortlist answered the wrong question.** The diagnosis of the cause is right and the conclusion about the shortlist is too strong. tiny-skia is a CPU **raster** compositor with the full W3C blend set exposed for pixmap-on-pixmap compositing (`src/shaders/pattern.rs:32-49`), and resvg decodes and composites raster images. The shortlist was not screening a disjoint universe; **it was screening the right libraries against the wrong criterion.** §2.5.
...
10. **An "apparent key swap" in ImageMagick's PSD writer table was a misread.** `coders/psd.c:281-284` returns `" mul"` for `LuminizeCompositeOp` and `" lum"` for `MultiplyCompositeOp`, which reads as a swap in isolation. Every row of that table returns the four-character key byte-reversed for LSB endian — `"vidi"/"idiv"`, `"krad"/"dark"`, `"dums"/"smud"`, `"mron"/"norm"` — and `" mul"` is `"lum "` reversed while `" lum"` is `"mul "` reversed; `ReadPSDString` at `:911-918` does the same reversal on read. The transferable rule: before calling a table entry wrong, read the whole table for its encoding rule.
11. **Two reported release dates conflicted and were re-verified at the registry.** `psd-tools` 1.17.4 uploaded 2026-06-24 (PyPI); ag-psd `latest` 31.0.2 published 2026-07-02 (npm), ahead of the newest git tag. Registry values are used throughout, because git tags lag publishes.
...
| **GraphicsMagick's alpha semantics, blend formula conformance, and last commit date** | A Mercurial checkout; the git mirror 404s. Licence, release date and the 1.3.24 PSD change are now verified from the project's own pages |
| **SVG 2's Candidate Recommendation date.** The two zero-occurrence checks are verified — see §1.10 — but the chapter documents supplied carry no publication date, and `w3.org` returns HTTP 403 from this environment | A fetch of the SVG 2 status section, or any snapshot that records its own date |
| **Skia's release dates.** It has milestone branches and no semver releases | Chromium release metadata; not retrieved |
| **libkra `v0.3` release date**, and licence files for `jsora`, the `krita` crate, and `xcfreader` | Non-GitHub hosts or absent files; each needs a direct fetch. `shivshank/xcf-rs` is settled: `UNLICENSE` present, `license = "Unlicense"`, at `3e1ef2cfb10142c467013f678c152fdaeeba5b6d` |
