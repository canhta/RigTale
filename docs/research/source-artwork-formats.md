...
| **`xcf-rs`'s crates.io publish date for 0.5.0** | A crates.io API call; the host returns HTTP 403 from this environment |
| **SVG 2's Candidate Recommendation date and the zero-occurrence checks over `struct.html` and `pservers.html`** | A fetch of `w3.org/TR/SVG2/`; the host returns HTTP 403 from this environment. The negatives were re-derived against the `w3c/svgwg` editor's draft, which is a different snapshot |
