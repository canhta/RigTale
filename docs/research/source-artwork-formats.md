...
**GraphicsMagick** — `http://www.graphicsmagick.org/`. No clone: the git mirror 404s and no Mercurial checkout was taken, so **nothing in this record is read from source**. Release 1.3.48 on 2026-07-23, the MIT-style `Copyright.html`, and the PSD drop at 1.3.24 are all taken from the project's own web pages and are discovery signals, **not verified**. Blend modes present by name; formula conformance and alpha semantics not verified. Weakest evidence base of the set.
...
**OpenColorIO** — `https://github.com/AcademySoftwareFoundation/OpenColorIO`. **BSD-3-Clause**, read from `LICENSE` on `main`; **no clone was pinned and the tag date for v2.5.2 is not verified**, so this record does not meet the pinning standard of the rest of §2.6. **Not a compositing library and contributes zero blend modes** — but blend results are colour-space dependent, so it is the natural home for the working-space half of the `TODO.md` colour-management gap.
...
| **GraphicsMagick, libmypaint** | `reject` | Dropped PSD at 1.3.24 and no source read / brush-dab library with no PDF blend modes. | — |
